...
LIA comes in two battery configurations, tied to the board/enclosure revision (see [Enclosure](/LIA/docs/hardware/enclosure/)):

- **V1** boards pair with a single 18650 Li-ion cell, held in a Keystone 54 clip on the main board (see [BOM](/LIA/docs/hardware/bom/)) — 3500 mAh.
- **V2** boards pair with a 50×37×4 mm pouch cell — 1000 mAh, in exchange for a much more compact enclosure.
...
Rather than a status LED pattern, LIA reports charge state as mesh text messages on its private channel (see [Meshtastic Configuration](/LIA/docs/firmware/meshtastic-configuration/)):
...
Send `CHG OFF` / `STB OFF` on the private channel to mute either message (or `CHG ON` / `STB ON` to re-enable) — see [Meshtastic Configuration](/LIA/docs/firmware/meshtastic-configuration/) for the full command list.
...
Send `BATTERY` on the private channel to get the current percentage on demand.
